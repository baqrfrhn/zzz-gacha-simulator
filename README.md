...
* Simulates an **Agent** banner, a **W-Engine** banner, or an **Agent ==>[source: 1] W-Engine** combo (finish the agent target first, then the w-engine target).
...
* PMF ==>[source: 1] CDF toggle with snap-to-point hover readouts, a pull-budget marker, and a per-run pull-history viewer.
...
* **miss** ==>[source: 1] advance the pity counter;
* **hit with no guarantee** ==>[source: 1] win with probability `winRate` (gain a copy, reset pity) or lose with `1 - winRate` (set guarantee, reset pity);
* **hit with guarantee** ==>[source: 1] guaranteed win.
...
### Combo mode (Agent ==>[source: 1] W-Engine)
...
* **`history_modal.cpp`** — a popup table of every Monte Carlo run: run number, total pulls + event count, and the win/loss chain (e.g. `Won at 78 ==>[source: 1] Lost at 74 ==>[source: 1] Won at 80`). Combo mode shows Agent and W-Engine tabs, with a "jump to sim #" control, scroll syncing between tabs, and `ImGuiListClipper` for large trial counts.
...
* [rlImGui](https://github.com/raylib-extras/rlImGui) (ImGui ==>[source: 1] raylib glue)
